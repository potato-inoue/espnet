...
for n in 8470; do # 1210 2420 3630 4840 6050 7260 8470 9680 10890 12100
    ...
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        utils/subset_data_dir.sh --first data/${trans_type}_train ${n} data/${train_set}
        dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta false \
            data/${train_set}/feats.scp data/${trans_type}_train_no_dev/cmvn.ark exp/dump_feats/${train_set} ${feat_tr_dir}
    fi

    # dict=data/lang_1${trans_type}/${train_set}_units.txt
    dict=data/lang_1${trans_type}/${trans_type}_train_no_dev_units.txt
