...
for n in 710 1420 2130 ; do #2840 3550 4260 4970 5680 6390 7196 

# train_set="${trans_type}_train_no_dev"
train_set="${trans_type}_train_${n}"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    utils/subset_data_dir.sh --last data/${trans_type}_train ${n} data/${train_set}
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta false \
        data/${train_set}/feats.scp data/${trans_type}_train_no_dev/cmvn.ark exp/dump_feats/${trans_type}_train_${n} ${feat_tr_dir}
fi

# dict=data/lang_${trans_type}/${train_set}_units.txt
dict=data/lang_${trans_type}/${trans_type}_train_no_dev_units.txt
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        apply-cmvn --norm-vars=true --reverse=true data/${trans_type}_train_no_dev/cmvn.ark \
