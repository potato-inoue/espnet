...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    # Update json
    for name in ${train_set} ${dev_set} ${eval_set}; do
        mkdir -p ${dumpdir}/${name}_xvec
        cp ${dumpdir}/${name}/data.json ${dumpdir}/${name}_xvec/data.json
        local/update_json.sh ${dumpdir}/${name}_xvec/data.json ${nnet_dir}/xvectors_${name}/xvector.scp
    done
fi
